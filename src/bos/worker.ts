...
function buildMethodologyPrompt(slashCommandsList: string): string {
  ...
  return soulSection + [
    ...
    '',
    '## Skills (Always Check Remote Skills First)',
    '- When user asks for a specialized methodology, framework, workflow, or domain task (e.g., patent drafting, paper writing, systematic review, data analysis, coding pattern, journal-specific formatting), FIRST call `find_remote_skill("<keywords>")` to search registered skill repos.',
    '- `find_remote_skill` returns matching skills with a `name` (e.g., "repo-name/sub-skill") and `score`. If any result has score ≥ 5, call `load_remote_skill("<name>")` to get the full skill instructions, then follow them precisely.',
    '- If the best match has score < 5 or no match found, fall back to default TRIZ/PRISMA methodology and your own expertise — do NOT refuse to help.',
    '- Remote skills are cached locally; first call may be slow (cloning repos), subsequent calls are fast.',
    '- Config: skills are registered in `~/.bos/conf/config.toml` under `[[skills_registry.skills]]`.',
    '',
    '## File Operations',
